-- Phase 0 & 1: Enterprise Multi-Tenant Schema Foundation (Fixed RLS)
-- ========================================================
...
-- ========================================================
-- SIMPLIFIED RLS POLICIES (Fixed column references)
-- ========================================================
...
CREATE POLICY "dept_org_access" ON departments
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = departments.org_id
    )
  );
...
CREATE POLICY "team_org_access" ON teams
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = teams.org_id
    )
  );
...
CREATE POLICY "profile_org_access" ON profiles
  ...
  USING (
    EXISTS (
      ...
      WHERE user_profile.id = auth.uid() AND user_profile.org_id = profiles.org_id
    )
  );
...
CREATE POLICY "membership_org_access" ON memberships
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = memberships.org_id
    )
  );
...
CREATE POLICY "customers_org_access" ON customers
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = customers.org_id
    )
  );
...
CREATE POLICY "quotes_org_access" ON quotes
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = quotes.org_id
    )
  );
...
CREATE POLICY "projects_org_access" ON projects
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = projects.org_id
    )
  );
...
CREATE POLICY "work_orders_org_access" ON work_orders
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = work_orders.org_id
    )
  );
...
CREATE POLICY "documents_org_access" ON documents
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = documents.org_id
    )
  );
...
CREATE POLICY "shipments_org_access" ON shipments
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = shipments.org_id
    )
  );
...
CREATE POLICY "time_entries_org_access" ON time_entries
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = time_entries.org_id
    )
  );
...
CREATE POLICY "notifications_org_access" ON notifications
  ...
  USING (
    EXISTS (
      SELECT 1 FROM profiles p 
      WHERE p.id = auth.uid() AND p.org_id = notifications.org_id
    )
  );
